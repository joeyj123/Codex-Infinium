...
function findMatchAt(text, pos, excludeTermId) {
  for (const candidate of MATCH_LIST) {
    if (excludeTermId && candidate.term.id === excludeTermId) continue;
...
// once per page, not on every repetition. `excludeTermId`, when given,
// skips that one term (and only that one) as a match candidate — used so a
// term's own definition never links back to itself.
export function highlightText(text, seenIds, excludeTermId) {
  if (!text) return [text];
  const segments = [];
  let i = 0;
  let bufferStart = 0;
  ...
  while (i < text.length) {
    const match = findMatchAt(text, i, excludeTermId);
    if (!match) {
      i += 1;
      continue;
    }
