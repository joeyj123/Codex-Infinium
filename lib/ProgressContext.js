...
export function ProgressProvider({ children, kb }) {
  ...
  // Workshop challenges (reorder/fix code) award XP the same way Forge
  // examples do, but are tracked under their own completion list — a
  // Workshop challenge and a Forge example aren't the same unit even when
  // they live on the same topic, so they shouldn't share one flag.
  function hasCompletedWorkshopChallenge(challengeId) {
    return progress.completedWorkshopChallenges.includes(challengeId);
  }

  function markWorkshopChallengeComplete(challengeId, xpAmount, tierName) {
    if (progress.completedWorkshopChallenges.includes(challengeId)) return 0;
    ...
    setProgress((p) =>
      p.completedWorkshopChallenges.includes(challengeId)
        ? p
        : {
            ...p,
            completedWorkshopChallenges: [...p.completedWorkshopChallenges, challengeId],
            xp: p.xp + xpAmount,
          }
    );
  ...
  return (
    <ProgressContext.Provider
      value={{
        ...
        hasCompletedWorkshopChallenge,
        markWorkshopChallengeComplete,
      }}
    >
      {children}
    </ProgressContext.Provider>
