...
export function ProgressProvider({ children, kb }) {
  ...
  // Anvil challenges (reorder/fix code) award XP the same way Forge
  // examples do, but are tracked under their own completion list — an
  // Anvil challenge and a Forge example aren't the same unit even when
  // they live on the same topic, so they shouldn't share one flag.
  function hasCompletedAnvilChallenge(challengeId) {
    return progress.completedAnvilChallenges.includes(challengeId);
  }

  function markAnvilChallengeComplete(challengeId, xpAmount, tierName) {
    if (progress.completedAnvilChallenges.includes(challengeId)) return 0;
    ...
    setProgress((p) =>
      p.completedAnvilChallenges.includes(challengeId)
        ? p
        : {
            ...p,
            completedAnvilChallenges: [...p.completedAnvilChallenges, challengeId],
            xp: p.xp + xpAmount,
          }
    );
