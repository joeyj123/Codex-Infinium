...
// lazy-loaded Pyodide worker) — everything else (Java/C#/C++) has no
// realistic in-browser execution path, so those tracks fall back to an
// authored "expected output" preview plus the same offline prose-grading
// engine used elsewhere in The Forge, applied to the submitted code text.
const LIVE_LANGS = ["python", "javascript"];
...
export default function ForgeTopicPage() {
  ...
  // language-track pages are always code topics (Python/JS run live, other
  // languages fall back to an authored preview), while Apprentice topics are
  // plain reasoning examples UNLESS the specific example carries its own
  ...
  const isLive = isExpert ? LIVE_LANGS.includes(lang) : example?.language === "python";
  ...
  async function runCode() {
    ...
    setRunning(true);
    const execResult = runLang === "python" ? await runPython(answerText) : await runJavaScript(answerText);
    setRunning(false);
    setCodeOutput(execResult);
  }

  // Grading branches by language: Python/JS actually execute the submitted
  // code in a sandboxed worker and compare real captured output against
  // expected_output (see lib/gradeCode.js). Everything else — prose answers,
  // and Java/C#/C++ code, which has no in-browser execution path — falls
  // back to lib/grading.js's offline word-overlap pattern-matching, same as
  // before Forge B. Submitting is what satisfies Challenge and Gauntlet's
  // "answer before solution" gate; Guided's box is optional reinforcement.
            ...
            <div style={{ marginTop: 12 }}>
              ...
              {!isLive && isExpert && (
                <p className="stat-line" style={{ marginTop: 6, color: "var(--muted)" }}>
                  {tier.language_tracks[lang]?.name || "This language"} can't run live in the browser — this is an
                  authored preview of what the worked solution prints, not a live result.
                </p>
              )}
            </div>
          )}
